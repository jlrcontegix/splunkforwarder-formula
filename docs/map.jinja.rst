...
We will see a quick setup to configure the ``splunkforwarder`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``splunkforwarder`` formula
`````````````````````````````````````````````````````

Now, we can provides the per DNS domain name configuration files for the ``splunkforwarder`` formulas under ``/srv/salt/splunkforwarder/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/splunkforwarder/parameters/dns:domain/
    ln -s dns:domain /srv/salt/splunkforwarder/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/splunkforwarder/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/splunkforwarder/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``splunkforwarder`` formula
``````````````````````````````````````````````````````

Now, we can provides the per role configuration files for the ``splunkforwarder`` formulas under ``/srv/salt/splunkforwarder/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/splunkforwarder/parameters/roles

We will define 2 roles:

- ``splunkforwarder/server``
- ``splunkforwarder/client``

We create a configuration for the role ``splunkforwarder/server`` in ``/srv/salt/splunkforwarder/parameters/roles/splunkforwarder/server.yaml``:
...
We create another configuration for the role ``splunkforwarder/client`` in ``/srv/salt/splunkforwarder/parameters/roles/splunkforwarder/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles splunkforwarder/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles splunkforwarder/client
...
.. code-block:: console
    ...
    mv /srv/salt/splunkforwarder/parameters/dns:domain/  '/srv/salt/splunkforwarder/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-splunkforwarder-as-json:
      test.nop:
        - name: {{ splunkforwarder | json }}
...
Here is an example based on `template-formula/splunkforwarder/config/file.sls`_:
...
.. code-block:: sls
    ...
    splunkforwarder-config-file-file-managed:
      file.managed:
        - name: {{ splunkforwarder.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='splunkforwarder-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ splunkforwarder.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``splunkforwarder`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    {{ splunkforwarder | json }}

    winner of the merge: {{ splunkforwarder['winner'] }}
...
.. _template-formula/splunkforwarder/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/splunkforwarder/config/file.sls
